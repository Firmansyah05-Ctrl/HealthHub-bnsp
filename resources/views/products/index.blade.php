...
@section('title', 'Perlengkapan Medis - Produk Kesehatan Profesional')
@section('description', 'Jelajahi koleksi leng                        <!-- Add to Cart Button -->an medis dan peralatan kesehatan profesional. Produk berkualitas untuk penyedia layanan kesehatan.')
            ...
            <ul class="space-y-1">
                <li>
                    <a href="{{ route('products.index') }}" 
                       class="flex items-center py-3 px-4 rounded-xl transition-all duration-200 {{ !$selectedCategory ? 'bg-teal-50 text-teal-600 border-l-4 border-teal-600' : 'hover:bg-gray-50 text-gray-700' }}">
                        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                        </svg>
                        Semua Produk
                        <span class="ml-auto bg-gray-100 text-gray-600 text-xs font-semibold px-2 py-1 rounded-full">{{ $totalProducts }}</span>
                    </a>
                </li>
                @foreach($categories as $category)
                <li>
                    <a href="{{ route('products.index', ['category' => $category->slug]) }}" 
                       class="flex items-center py-3 px-4 rounded-xl transition-all duration-200 {{ $selectedCategory == $category->slug ? 'bg-teal-50 text-teal-600 border-l-4 border-teal-600' : 'hover:bg-gray-50 text-gray-700' }}">
                        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                        </svg>
                        {{ $category->name }}
                        <span class="ml-auto bg-gray-100 text-gray-600 text-xs font-semibold px-2 py-1 rounded-full">{{ $category->products_count }}</span>
                    </a>
                </li>
                @endforeach
            </ul>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 mb-2">
                    @if($selectedCategory)
                        {{ $categories->where('slug', $selectedCategory)->first()->name ?? 'Produk' }}
                    @else
                        Semua Produk
                    @endif
                </h2>
            ...
            <div class="flex items-center space-x-4 mt-4 sm:mt-0">
                <form method="GET" action="{{ route('products.index') }}" class="flex items-center search-form" id="sortForm">
                    @if($selectedCategory)
                ...
                <div class="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden group border border-gray-100 hover:border-teal-200 hover:scale-[1.02] transform">
                    ...
                    <div class="p-7 space-y-5">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="space-y-2">
                                <p class="text-gray-600 text-sm leading-relaxed line-clamp-3" id="desc-{{ $product->id }}">
                                    {{ $product->description }}
                                </p>
                        ...
                        @else
                            <!-- Add to Cart Form -->
                            <form action="{{ route('cart.add', $product->id) }}" method="POST" class="w-full pt-4">
                                @csrf
                                <button type="submit" 
            ...
            <div class="text-center py-20">
                <div class="max-w-lg mx-auto">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        <a href="{{ route('products.index') }}" class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-teal-600 to-cyan-600 text-white font-bold rounded-2xl hover:from-teal-700 hover:to-cyan-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
